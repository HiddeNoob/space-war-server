...
export class PacketHandler {
    ...
    constructor() {
        ...
        // Register to network data stream
    ...
    _decodePacketInternal(dataView: DataView, offset: number): { decodedObject: any; newOffset: number; type?: PacketType } {
        ...
        if (decoder) {
            ...
            newOffset = offset + bodySize; // Skip unknown body
        }
    ...
    decodeUniformListPacket(dataView: DataView, offset: number, bodySize: number): { decodedObject: any; newOffset: number } {
        ...
        switch (itemTypeId) {
            ...
            case PacketType.BREAKABLE_LINE: itemBodySize = 20; break;
            default:
        ...
        for (let i = 0; i < itemCount; i++) {
            ...
            } else if (itemTypeId === PacketType.BREAKABLE_LINE) {
                ({ decodedObject: decodedItem, newOffset: itemNewOffset } = this.decodeBreakableLinePacket(dataView, currentOffset));
            } else {
                break;
            }
    ...
    encode(packetType: PacketType, payload: ActionPacket): ArrayBuffer | null {
        if (packetType !== PacketType.ACTION) {
            console.error(`Encoder only supports ACTION packet at this moment.`);
            return null;
        }
        ...
        if (payload.force) {
            mask |= ActionType.FORCE;
            bodySize += 8;
        }
        if (payload.rotation) {
            mask |= ActionType.ROTATION;
            bodySize += 4;
        }
        if (payload.attack) {
            mask |= ActionType.ATTACK;
        }
        if (payload.boost) {
            mask |= ActionType.BOOST;
        }
